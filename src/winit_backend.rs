...
pub fn launch<T, M: SyncMainLoop<Args=T> + 'static>(info: AppInfo, user: T) -> Result<()> {
    ...
    begin_loop::<T, M>(core, event_loop, window, surface, present_mode, user)
}
...
fn begin_loop<T, M: SyncMainLoop<Args=T> + 'static>(
    core: Core,
    event_loop: EventLoop<()>,
    window: Window,
    surface: SurfaceKHR,
